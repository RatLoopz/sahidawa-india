 ...
 */
export async function aggregateRateLimit(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (redisClient.isOpen) {
        try {
            ...
            logger.error("[aggregateRateLimit] Redis eval failed, allowing request", { error: String(err) });
            return next();
        }
    } else {
        const now = Date.now();
