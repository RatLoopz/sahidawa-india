...
export default function ScanPage() {
    ...
    return (
        <div className="relative flex min-h-screen flex-col bg-black font-sans text-white">
            {/* Gallery picker — opens device file/gallery, never the camera directly */}
            <input
                ref={galleryInputRef}
                type="file"
                id="medicine-upload-gallery"
                className="hidden"
                accept="image/*"
                onChange={handleFileUpload}
            />
            {/* Camera capture — opens the device camera directly */}
            <input
                ref={cameraInputRef}
                type="file"
                id="medicine-upload-camera"
                className="hidden"
                accept="image/*"
                capture="environment"
                onChange={handleFileUpload}
            ...
            <div className="flex flex-col items-center gap-6 bg-linear-to-t from-black to-transparent p-8">
                ...
                <div className="flex gap-3">
                    {/* Upload Photo — triggers gallery/file picker only */}
                    <button
                        type="button"
                        onClick={() => galleryInputRef.current?.click()}
                        disabled={isScanning}
                        aria-label="Upload photo from gallery"
                        className="flex cursor-pointer items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-bold text-black shadow-lg transition-colors hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                        <ImageIcon size={18} />
                        Upload Photo
                    </button>
                    {/* Use Camera — opens device camera directly */}
                    <button
                        type="button"
                        onClick={() => cameraInputRef.current?.click()}
                        disabled={isScanning}
                        aria-label="Capture photo using camera"
                        className="flex cursor-pointer items-center gap-2 rounded-full border border-white/20 bg-white/10 px-6 py-3 text-sm font-bold text-white shadow-lg transition-colors hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                        <Camera size={18} />
                        Use Camera
                    </button>
