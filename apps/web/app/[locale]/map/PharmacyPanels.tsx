    ...
    return (
        <article
            className={`rounded-xl border p-3 transition-all duration-200 ${
                isSelected
                    ? "border-emerald-300 bg-emerald-50/60 shadow-md shadow-emerald-100/30 dark:border-emerald-700 dark:bg-emerald-950/40 dark:shadow-none"
                    : "border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"
            }`}
        >
            <button
                type="button"
            ...
            >
                <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                            ...
                            {pharmacy.isVerified && (
                                <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700 dark:bg-emerald-900 dark:text-emerald-400">
                                    <Shield size={7} />
                                    Verified
                                </span>
                            )}
                ...
                <div className="mt-1.5 ml-11 flex flex-wrap gap-1">
                    <span className="inline-flex items-center gap-0.5 rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                        <Shield size={6} />
                        {pharmacy.status}
                    </span>
                    <span
            ...
            {pharmacy.phone && (
                <div className="mt-2 ml-11">
                    <a
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600 transition-colors hover:bg-slate-200 active:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                    >
                        <Phone size={9} className="text-emerald-600 dark:text-emerald-400" />
                        Call
                    </a>
                </div>
            )}
...
export default function PharmacyPanels({
    ...
    return (
        <section
            className={`flex h-full flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white/96 shadow-xl backdrop-blur-xl transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 ${
                className || ""
            }`}
        >
            <div className="shrink-0 border-b border-slate-100/80 px-5 py-4 dark:border-slate-800">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto px-4 py-4">
                {isLoading ? (
                    <div className="py-10 text-center">
                        <Loader2
                            size={26}
                            className="mx-auto mb-3 animate-spin text-emerald-600 dark:text-emerald-400"
                        />
