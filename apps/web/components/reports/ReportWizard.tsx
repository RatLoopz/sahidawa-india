...
const schema = z.object({
  ...
  pharmacyName: z
    .string()
    .transform(sanitize)
    .pipe(z.string().min(2, "Required")),
  address: z
    .string()
    .transform(sanitize)
    .pipe(z.string().min(5, "Required")),
  ...
  Send: () => (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M1.5 7h11M8 2.5l4.5 4.5L8 11.5"
      />
    </svg>
  ),
  ShieldCheck: () => (
          ...
          return (
            ...
            >
              {done ? (
                ...
                <>
                  <span className={active ? "text-emerald-400" : "text-slate-300"}>
                    {s.n}
                  </span>
                  {s.code}
                </>
    ...
    <div className="space-y-5">
      ...
      >
        ...
        {busy ? (
          ...
          <>
            <span className="text-slate-400 bg-white p-3 rounded-xl shadow-sm border border-slate-100 mb-1">
              <Icon.Upload />
            </span>
            <div>
          ...
          <AnimatePresence>
            {images.map((img, idx) => (
              ...
              >
                ...
                {/* Remove overlay */}
                <div
                  className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100
                ...
                {/* Name bar */}
                <div
                  className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-slate-900/80 px-2 py-1.5
                  ...
                  {step < 3 ? (
                    ...
                    >
                      {submitting ? (
                        <>
                          <span
                            className="w-5 h-5 border-[3px] border-white/30 border-t-white
