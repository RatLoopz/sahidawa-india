    ...
    <AnimatePresence>
      {isListeningForIntent && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] w-11/12 max-w-sm rounded-2xl bg-slate-900/95 p-6 shadow-2xl backdrop-blur-md border border-slate-700"
        >
      ...
      {!isListeningForIntent && (
        <button
          ...
          className={`fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full shadow-lg transition-colors ${
            isWakeWordActive ? 'bg-emerald-600 text-white animate-pulse' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white'
          }`}
          aria-label={isWakeWordActive ? "Deactivate Voice Assistant" : "Activate Voice Assistant"}
        >
          <Mic size={24} />
        </button>
      )}
