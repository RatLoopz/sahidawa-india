...
2.  **Strict Creator-Only Assignment:**
    ...
    - It checks if the issue is currently unassigned (`isUnassigned`) and if it _does not_ have the `status: open-for-all` label. The `hasOpenForAllLabel` flag is determined by iterating `issueData.data.labels` and checking `l.name` (or `l` if a string) for a case-insensitive match.
...
3.  **JavaScript String Manipulation for Spam Detection:**
    - To clean comment bodies:
        ...
        const cleanedBody = rawBody
            .replace(/\r?\n|\r/g, " ")
            .replace(/\/assign|please assign me|assign me/gi, "")
            .trim();
    ...
    - To detect bot spam patterns:
        ...
        if (
            botBody.match(
                /(⚠️|You've been assigned|Assignment Request Denied|already assigned|Auto-unassigned|inactivity)/i
            )
        ) {
            // This is bot spam
        }
