...
2.  **Server-side Authorization Guard:** A dedicated server-side layout guard (`apps/web/app/[locale]/admin/layout.tsx`) was introduced. This guard performs a granular authorization check _after_ middleware authentication. It retrieves the user's role (from `session.user.app_metadata` or `session.user.user_metadata`) and strictly verifies if it is either `admin` or `moderator`. If the user's role does not meet these requirements, a 403 "Access Denied" error component is rendered instead of the admin dashboard content.
